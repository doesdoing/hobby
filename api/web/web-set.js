                ...
                MySql.callback(data, sql_info.check_seesion_cookies, [req.session.user_id]).then(DATA => {
                    if (DATA[0].level == 'admin' || DATA[0].level == 'user') {
                        if (req.query.DEL_ID == '1') {
                            ...
                            VALUE = ['system_web',req.body.ID];
                        } else if (req.query.SET_ID == '1') {
                            SQL_CMD = sql_info.web_sql_set;
                            VALUE = [req.body.web1, req.body.web2, req.body.admin_name, req.body.password, req.body.system_name, req.body.ID];
                        } else if (req.query.ADD_ID == '1') {
                            SQL_CMD = sql_info.web_sql_add;
                            VALUE = [req.body.system_name, req.body.web1, req.body.web2, req.body.admin_name, req.body.password];
                        }
                        MySql.callback(data, SQL_CMD, VALUE, 'T').then(D => {
                            res.status(200).redirect('/system-web-list');
                        });
                    } else {
